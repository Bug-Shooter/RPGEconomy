...
public class SettlementsBuildingsMarketsApiTests : IAsyncLifetime
{
    ...
    public async Task Settlement_Building_And_Market_Endpoints_Should_Work_EndToEnd()
    {
        ...
        var productId = await seed.CreateProductTypeAsync("Bread", "Desc", 10, 1);
        ...
        var marketResponse = await client.PostAsJsonAsync($"/api/settlements/{settlement.SettlementId}/market/products", new { productTypeId = productId, initialPrice = 12.5 }, cancellationToken: TestContext.Current.CancellationToken);
    ...
    private sealed record MarketPriceResponse(int ProductTypeId, string ProductName, double Price, int Supply, int Demand);
}
